...
BEGIN
    ...
    SET totalmoney = (SELECT SUM(CD.PRICE)
                      ...
                      WHERE C.USER_NAME = p_USERNAME AND C.STATE = 0);
    UPDATE CART
    SET CART.TOTAL_MONEY = totalmoney
    WHERE CART.USER_NAME = p_USERNAME AND CART.STATE = 0;
END;
...
BEGIN
    ...
    WHERE C.USER_NAME = p_USER_NAME AND C.STATE = 0;
END;
...
BEGIN
    ...
    ELSE
        SET num=0;
        SET cart_id = (SELECT ID FROM CART WHERE USER_NAME = p_USERNAME AND CART.STATE= 0);
        ...
        IF (not EXISTS  (SELECT NO_CART FROM CART_DETAIL WHERE CART_ID = cart_id AND PRODUCT_ID = p_PRODUCT_ID)) THEN
...
--CALL ADD_CART ('bk102', 'test123', @p_mes); SELECT @p_mes;
...
CREATE PROCEDURE ORDERT(IN p_USER_NAME VARCHAR(30), IN p_FULLNAME NVARCHAR(100), IN p_PHONE_NUMBER VARCHAR(10), IN p_ADDRESS NVARCHAR(100))
BEGIN
    DECLARE cartID INT;
    SET cartID = (SELECT ID FROM CART WHERE CART.USER_NAME = p_USER_NAME AND CART.STATE = 0);
    
    INSERT INTO CART(USER_NAME,TOTAL_MONEY, STATE)
    VALUES (p_USER_NAME, 0, FALSE);

    UPDATE CART 
    SET CART.STATE = 1
    WHERE CART.ID = cartID;
    INSERT INTO ORDERT(CART_ID, USER_NAME, FULLNAME, ADDRESS, PHONE_NUMBER)
    VALUES (cartID, p_USER_NAME, p_FULLNAME, p_ADDRESS, p_PHONE_NUMBER);  
END;
DROP PROCEDURE ORDERT
--- 
...
BEGIN
    DECLARE result INT;
    SET result = (SELECT TOTAL_MONEY FROM CART WHERE USER_NAME = Username AND CART.STATE = 0);
    RETURN result;
END;
...
CREATE PROCEDURE CREATE_ACOUNT(IN p_USERNAME NVARCHAR(20), IN p_PASSWORD VARCHAR(300), IN p_MAIL VARCHAR(50), OUT p_Message NVARCHAR(255))
BEGIN 
    DECLARE v_Id INT;
...
CREATE PROCEDURE SEARCH_PRODUCT(IN p_PRODUCT_NAME NVARCHAR(200))
(
    SELECT *
    FROM PRODUCT
    WHERE PRODUCT.PRODUCT_NAME LIKE CONCAT('%', p_PRODUCT_NAME, '%')
);
