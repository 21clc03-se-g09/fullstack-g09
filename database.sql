...
-- THÊM SẢN PHẨM (ADMIN)
CREATE PROCEDURE ADD_NEW_PRODUCT(IN p_PRODUCT_ID INT, IN p_PRODUCT_NAME NVARCHAR(100), p_AUTHOR NVARCHAR(50), p_CATEGORY NVARCHAR(30), p_QUANTITY INT,  p_PUBLISHED_DATE DATETIME, p_DESCRIBE TEXT, p_IMGE_URL TEXT, OUT p_Message NVARCHAR(100))
BEGIN
    IF ((SELECT * FROM PRODUCT WHERE ID = p_PRODUCT_ID) IS NOT NULL) THEN
        SET p_Message = 'SẢN PHẨM ĐÃ TỒN TẠI';
    ELSE
        INSERT INTO PRODUCT
        VALUES (p_PRODUCT_ID, p_PRODUCT_NAME, p_AUTHOR, p_CATEGORY, p_QUANTITY, p_PUBLISHED_DATE, p_PUBLISHED_DATE, p_DESCRIBE, p_IMGE_URL);
